...
- Completed increments: BC0-A contract/threat-model lock; BC0-B additive contract and conformance;
  ...
  current-main integration and recovery-dispatch validator repair
- Current increment: complete
- Resolved defect: recovery `dispatch_batch` now preserves the exact reconciliation claim across the
  dispatched snapshot; the allowlist remains enforced at the recovery-state command gate. See
