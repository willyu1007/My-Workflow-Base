...
function assertLock(expected, actual, lockPath) {
  ...
  for (const root of sourceRoots) {
    for (const file of sourceFiles(root.physicalRoot)) {
      ...
      const currentSource = readFileSync(file, "utf8").replace(/^\uFEFF/, "").replace(/\r\n?/g, "\n");
      if (committedSource !== currentSource) {
        throw new Error(`workflow contract source revision does not contain current ${repositoryPath}: ${lockPath}`);
      }
    }
  }
}
