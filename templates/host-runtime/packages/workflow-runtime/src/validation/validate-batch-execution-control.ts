...
function validateSnapshot(value: unknown, path: string): void {
  ...
  if (stateClaim.kind === "reconciliation_only") {
    const allowedForState = reconciliationClaimOperationsByRecoveryState[state];
    if (allowedForState === undefined) {
      fail("invalid_scalar", `${path}.claim.kind`, "Reconciliation claims are permitted only in recovery states.");
    }
    for (let index = 0; index < (stateClaim.allowed_operations as unknown[]).length; index += 1) {
      const operation = (stateClaim.allowed_operations as unknown[])[index] as string;
      if (!allowedForState.has(operation)) {
        fail(
          "invalid_scalar",
          `${path}.claim.allowed_operations[${index}]`,
          `Recovery state ${state} does not permit claim operation ${operation}.`,
        );
      }
    }
  }
