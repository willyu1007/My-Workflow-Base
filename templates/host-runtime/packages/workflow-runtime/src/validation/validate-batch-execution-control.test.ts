...
describe("decodeWorkflowBatchExecutionControlJsonV1", () => {
  ...
  it("preserves a recovery dispatch claim across the dispatched snapshot", () => {
    const requestSeed = withRequestHashForTest({
      ...recoveryCommon,
      operation: "dispatch_batch",
      current_claim: sealedRecoveryClaim,
      precondition: {
    ...
    const { request, receipt } = authorityPairForRequest(
      requestSeed,
      "dispatch_recorded",
  ...
  it("closes recovery snapshot unions, reconciliation roots, and recovery claim shapes", () => {
